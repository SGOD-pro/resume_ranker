...
1. **Local-First:** Do not write SQS push logic until Phase 4. Phase 2 uses FastAPI `BackgroundTasks` to simulate async processing locally. Local-First applies to SQS/Lambda A-B deployment only — ODL parsing and Nova fallback always hit real AWS. See NFR-01.
...
8. **ODL Standalone & Local-Cloud Split:** ODL is a standalone cloud Lambda (`odl-parser-lambda`). Called via `boto3`. In local dev, S3/DB are floci, but Lambda/Bedrock calls hit real AWS.
