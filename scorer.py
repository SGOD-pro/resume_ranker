...
# Comprehensive alias map: abbreviations, typos, alternate spellings
...
def _skill_match(candidate_skill: str, jd_skill: str) -> bool:
    ...
    cn = _normalize_skill(candidate_skill)
    jn = _normalize_skill(jd_skill)
    ...
    if len(cn) <= 1 or len(jn) <= 1:
        ...
        cn_resolved = _SKILL_ALIASES.get(cn, cn)
        jn_resolved = _SKILL_ALIASES.get(jn, jn)
...
def _find_matching_skills(candidate_skills: List[str],
    ...
    for jd_sk in jd_skills:
        found = any(_skill_match(c_sk, jd_sk) for c_sk in candidate_skills)
        if found:
...
def _most_recent_end_date(experience: List[Dict[str, Any]]) -> Optional[datetime]:
...
# ─────────────────────────────────────────────────────────────────────────────
# Text Similarity (TF-IDF Cosine — lightweight, no deps)
# ─────────────────────────────────────────────────────────────────────────────
...
def _text_cosine_sim(text1: str, text2: str) -> float:
    """Cosine similarity between two text strings."""
    return _cosine_sim(_tf(_tokenize(text1)), _tf(_tokenize(text2)))
...
def _bm25_skill_score(candidate_skills: List[str],
                      jd_skills: List[str],
...
def _experience_score(candidate: Dict[str, Any],
                      jd: JobDescription) -> Tuple[float, str]:
    """
    ...
    # ── Title similarity ──────────────────────────────────────────────────
    jd_title_tokens = _tf(_tokenize(jd.title))
    best_sim = 0.0
    best_title = ""
    for exp in experience:
        role = exp.get('role') or ''
        sim = _cosine_sim(_tf(_tokenize(role)), jd_title_tokens)
        if sim > best_sim:
            best_sim = sim
            best_title = role
    ...
    # ── Recency ───────────────────────────────────────────────────────────
    most_recent = _most_recent_end_date(experience)
    if most_recent:
...
def _keyword_score(candidate: Dict[str, Any],
                   jd: JobDescription) -> Tuple[float, List[str], List[str]]:
    """
...
def _education_score(candidate: Dict[str, Any],
                     jd: JobDescription) -> Tuple[float, str, str]:
    """
    ...
    for edu in education:
        ...
        level = _parse_degree_level(degree)
        if level > best_level:
            best_level = level
            best_degree = degree
    ...
    if not jd.preferred_field or jd.preferred_field.lower() in ('any', '', 'related'):
        ...
        for edu in education:
            ...
            sim = _text_cosine_sim(degree_text, jd.preferred_field)
